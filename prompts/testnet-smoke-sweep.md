...
Issue-agnostic smoke test of the `testnet` binary against its built-in
scenarios (see `testnet --help`) and a few exploratory variants. Run the scenarios as
...
In a clean checkout of `boule` at `origin/main` HEAD, from the
repo root:

```sh
cargo build --release --bin testnet --bin boule
mkdir -p /tmp/regression-runs
BIN=$(pwd)/target/release/boule
...
- Always pass `--boule-bin $BIN` explicitly to `new`, `up`, and `scenario`
  invocations.
...
- After every scenario, also run the **back-pressure check** below. Any
  ...
  failure to flag. (See `BackpressureStatus` in
  `crates/boule-consensus/src/status.rs`; the policy is that
...
$TESTNET new --nodes 4 --seed 42 --workdir $WD --boule-bin $BIN
$TESTNET scenario rotating-failure --f 1 --seed 42 --workdir $WD --boule-bin $BIN
...
$TESTNET new --nodes 4 --seed 42 --workdir $WD --boule-bin $BIN
$TESTNET up   --workdir $WD --boule-bin $BIN
...
$TESTNET up   node2                 --workdir $WD --boule-bin $BIN
...
$TESTNET new --nodes 7 --seed 42 --workdir $WD --boule-bin $BIN
$TESTNET scenario disconnect-random --count 2 --liveness-window 5 --seed 42 --workdir $WD --boule-bin $BIN
$TESTNET snap --workdir $WD
$TESTNET verify-safety --workdir $WD
$TESTNET telemetry --workdir $WD
$TESTNET down --workdir $WD; rm -rf $WD
...
The demo `/messages` app was removed (#532); the gossip overlay now
carries only consensus traffic. This trial verifies the overlay is live
cluster-wide by confirming every node both commits (gossip delivered the
proposals/votes) and reports a non-zero gossip dispatch counter once the
mesh is healthy.

```sh
WD=/tmp/regression-runs/a4
$TESTNET new --nodes 7 --seed 1 --workdir $WD --boule-bin $BIN
$TESTNET up --workdir $WD --boule-bin $BIN
$TESTNET wait --all-healthy --within 5 --workdir $WD --timeout 30
$TESTNET wait --all-reach-height 10 --workdir $WD --timeout 30
$TESTNET snap --workdir $WD
$TESTNET verify-safety --workdir $WD
$TESTNET telemetry --workdir $WD
$TESTNET down --workdir $WD; rm -rf $WD
...
Pass: `wait --all-reach-height 10` exit 0 (gossip propagated proposals
and votes to a committing 5-of-7 quorum on all nodes); `verify-safety`
exit 0; `gossip_send_to_dispatched > 0` on every node in the post-trial
telemetry.

### A5. Workdir path probe — 1 trial

```sh
cd /tmp/regression-runs && $TESTNET new --nodes 4 --workdir a5 --boule-bin $BIN
cd / && $TESTNET up --workdir /tmp/regression-runs/a5 --boule-bin $BIN
...
`reconcile_bls_identity`
([crates/boule-node/src/lib.rs](crates/boule-node/src/lib.rs)) caught a
config inconsistency — capture the stderr in the failure dump.
...
```sh
$TESTNET new --nodes 7 --seed 7 --workdir $WD --boule-bin $BIN
$TESTNET scenario --file /path/to/scenario.toml --workdir $WD --boule-bin $BIN
echo "exit=$?"
```

This is a known footgun (documented on the scenario example in
`crates/boule-node/src/testnet/scenario.rs`): with seed=7+count=2 the
kill set isolates a node before gossip mesh expansion. Expected outcome: non-zero exit, `wait_all_reach_height(60)`
...
$TESTNET new --nodes 4 --seed $SEED --workdir $WD --boule-bin $BIN
$TESTNET scenario rotating-failure --f 1 --seed $SEED --workdir $WD --boule-bin $BIN
$TESTNET snap --workdir $WD
...
$TESTNET new --nodes 7 --seed $SEED --workdir $WD --boule-bin $BIN
$TESTNET up --workdir $WD --boule-bin $BIN
$TESTNET wait --all-reach-height 5 --workdir $WD --timeout 30
...
- **`PARTITIONED=true` + wedge → expected partial-sync stall, probe
  ...
  for n in $(grep '"kind":"kill"' $WD/events.jsonl | jq -r .node); do
    $TESTNET up $n --workdir $WD --boule-bin $BIN
  done
...
sleep 30
$TESTNET up node2 --workdir $WD --boule-bin $BIN
$TESTNET up node3 --workdir $WD --boule-bin $BIN
$TESTNET up node4 --workdir $WD --boule-bin $BIN
