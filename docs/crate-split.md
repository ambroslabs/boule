...
Splitting the monolithic `boule-core` library into layered crates. Tracks the
target layering, the cycles that forced specific moves, and **what is left
behind in `boule-core`**.

## Target crates (depend strictly downward)

```
boule-core            clock, crypto (incl. NodeId/NodeIdentity via identity, SignerBitmap),
...
(today's p2p impl)           — depends on boule-core only; NO edge to boule-transport-tcp
   ▲                         ▲
...
- `crypto → p2p` (`NodeId`/`NodeIdentity`) — identity moved to `boule-core::identity`
...
- `config ↔ {consensus, p2p}` — config split: raw structs + TOML loading + identity
  providers stay in `boule-core`; the conversion methods move to consumers
...
## What is LEFT BEHIND in boule-core (bottom primitives)
...
- **Step 1 (committed):** identity → `boule-core::identity`; `SignerBitmap` → `crypto`;
  genesis/`derive_chain_id` → `consensus::genesis`. boule-core compiles + lib tests pass.
- **Step 2 (committed):** config detangle — `DEFAULT_*` + `MAX_FRAME_BYTES` → `config`;
  ...
  code now has zero upward refs. boule-core compiles + lib tests pass.
...
- `boule-core/testing` — `ChainId::TEST` (the all-zero sentinel stays out of production).
...
`boule-node`'s dev-dependencies enable `boule-core/testing`,
...
- **boule-transport** ← `p2p/limits.rs` (as `limits.rs`) + `p2p/overlay/traits.rs`
  (as `overlay.rs`; holds Broadcaster/Discovery/DiscoveryEvent). Deps: boule-core, bytes,
...
- `crate::crypto::` → `boule::crypto::`; same for `clock`, `storage`, `config`,
  `paths`, `identity`, `cli`.
...
Compile order: boule-core → boule-transport → {boule-transport-tcp, boule-consensus} →
