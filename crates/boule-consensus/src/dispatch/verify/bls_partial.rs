...
//!
//! Every Vote must arrive carrying a valid BLS partial under the
//! signer's historical BLS pubkey at `view`. Folding a Vote with a
//! missing or invalid partial into a
...
/// Require an attached BLS partial signature on every inbound `Vote` and
/// verify it against the signer's BLS pubkey at `signed.payload.view`.
/// Under the `cfg(test)`-only `QcVerification::Skip` the optional
/// partial is ignored.
///
