...
///
/// # Verified typestate at the wire boundary (#371)
///
...
impl HotStuffCore {
    ...
    pub fn step(&mut self, event: Event) -> Vec<Action> {
        match event {
            Event::ProposalReceived(signed) => self.on_proposal_received(signed.into_inner()),
            Event::VoteReceived(signed, bls_partial) => {
                self.on_vote_received(signed.into_inner(), bls_partial)
            }
            Event::NewViewReceived(signed) => self.on_new_view_received(signed.into_inner()),
