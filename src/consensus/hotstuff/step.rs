...
use super::qc::{
    ConsensusMsg, LeaderEndorsement, NewView, Proposal, QuorumCertificate, VerifiedQc, Vote,
};
...
// `Verified<Signed<Proposal>>` carries a Block + QC + leader-endorsement
// envelope (#506); boxing would add an indirection on every wire-driven
// event for a marginal memory saving. Same trade-off as on `ConsensusMsg`.
...
pub enum VoteVariant {
    /// Ed25519 chain vote: signed envelope plus the leader-endorsement
    /// envelope (#506). No BLS partial.
    Ed25519(
        crate::consensus::dispatch::Verified<Signed<Vote>>,
        Signed<LeaderEndorsement>,
    ),
    /// BLS-aggregated chain vote: signed envelope, BLS partial, and the
    /// leader-endorsement envelope (#506). The BLS partial folds into
    /// the QC bucket via [`QuorumCertificate::add_bls_partial`]; the
    /// endorsement is verified separately against the leader's pubkey
    /// at `vote.view`.
    Bls {
        signed: crate::consensus::dispatch::Verified<Signed<Vote>>,
        partial: BlsPartialSig,
        leader_endorsement: Signed<LeaderEndorsement>,
    },
}
...
impl VoteVariant {
    ...
    pub fn verified(&self) -> &crate::consensus::dispatch::Verified<Signed<Vote>> {
        match self {
            VoteVariant::Ed25519(verified, _) => verified,
            VoteVariant::Bls { signed, .. } => signed,
        }
    }

    /// Borrow the leader-endorsement envelope (#506). Self-attributing:
    /// the envelope's `signer` field carries the proposer's pubkey.
    /// Aggregators resolve the leader's stable [`ValidatorId`] from
    /// that pubkey via `ValidatorKeyHistory::validator_for`. A vote
    /// whose endorsement does not verify against `signer` (or whose
    /// `signer` is not in the validator set at `vote.view`) is
    /// rejected before any
    /// `vote_dedupe`/`vote_bucket`/`proposal_endorsement_dedupe`
    /// mutation.
    ///
    /// [`ValidatorId`]: ValidatorId
    pub fn leader_endorsement(&self) -> &Signed<LeaderEndorsement> {
        match self {
            VoteVariant::Ed25519(_, endorsement) => endorsement,
            VoteVariant::Bls {
                leader_endorsement, ..
            } => leader_endorsement,
        }
    ...
    pub fn from_optional_partial(
        ...
        match bls_partial {
            Some(partial) => VoteVariant::Bls {
                signed,
                partial,
                leader_endorsement,
            },
            None => VoteVariant::Ed25519(signed, leader_endorsement),
        }
    }
}
...
pub enum Action {
    ...
    /// A leader minted two distinct proposals (different `block_hash`
    /// values) at the same view (#506). Detected on the **vote stream**:
    /// every accepted vote at view `v` carries a leader-endorsement
    /// signature that pins `(v, block_hash)` to the leader's stable id;
    /// when two such bindings collide for the same `(v, leader)` pair
    /// with different block_hashes, the safety core emits this variant
    /// before the conflicting partial is folded into the second bucket.
    ///
    /// Sibling to [`Self::EquivocationEvidence`] (voter equivocation)
    /// rather than a discriminator-tagged extension so downstream
    /// consumers (slashing pipeline, telemetry) get exhaustive `match`
    /// enforcement and field names that match the perpetrator's role:
    /// `leader` here vs `voter` there.
    ///
    /// Detection is transport-agnostic: a Byzantine leader who splits
    /// conflicting proposals across disjoint peer subsets still has
    /// every voter rebroadcasting the leader's endorsement attached to
    /// their vote, so any aggregator that sees votes from both halves
    /// fires this evidence — independent of whether the aggregator
    /// directly received both proposals.
    ///
    /// The integration layer ticks
    /// [`crate::consensus::status::ConsensusStatus::proposal_equivocations_detected`]
    /// and emits a structured WARN; a full slashing pipeline is future
    /// work.
    ...
    /// A vote arrived whose `leader_endorsement` field did not verify
    /// against the leader-of-`vote.view`'s pubkey (#506). Emitted before
    /// any `vote_dedupe` / `vote_bucket` / `proposal_endorsement_dedupe`
    /// state is touched — the vote is dropped on the floor at ingress.
    /// The integration layer ticks
    /// [`crate::consensus::status::ConsensusStatus::votes_rejected_invalid_endorsement`]
    /// and emits a WARN trace.
    VoteRejectedInvalidEndorsement { voter: ValidatorId, view: View },
}
...
pub struct HotStuffCore {
    ...
    /// Per-`(view, leader_id)` block-hash dedup for leader proposal
    /// equivocation (#506). Populated from every accepted vote's
    /// leader-endorsement: the first vote at `view` whose endorsement
    /// resolves to `leader_id` records its `vote.block_hash` here; any
    /// subsequent vote at the same view whose endorsement resolves to
    /// the same leader for a *different* `block_hash` is non-repudiable
    /// proposal-equivocation evidence and emits
    /// [`Action::ProposalEquivocationEvidence`].
    ///
    /// Detection works on the vote stream rather than on observed
    /// proposals so a Byzantine leader who splits conflicting proposals
    /// across disjoint peer subsets is still caught — any aggregator
    /// that sees votes from both halves fires the evidence emit.
    /// Garbage-collected alongside [`Self::vote_dedupe`] in
    /// [`Self::evict_vote_buckets_below`] under the same `gc_below`
    /// cutoff so a Byzantine flood of low-view distinct-hash votes
    /// can't pin memory.
    proposal_endorsement_dedupe: HashMap<(View, ValidatorId), BlockHash>,
    /// Verified leader-endorsement signatures for `(view, block_hash)`
    /// pairs already validated this run. Populated on every successful
    /// vote ingress so subsequent votes carrying the same endorsement
    /// skip the Ed25519 verify call. Stored as the resolved leader's
    /// stable [`ValidatorId`] plus the 64 sig bytes — checking equality
    /// against an inbound `(signer, sig)` is enough to short-circuit.
    /// Bounded by the same cap as [`Self::vote_bucket`] (one bucket
    /// per `(view, block_hash)` pair, one cache entry alongside it).
    proposal_endorsement_cache: HashMap<(View, BlockHash), (ValidatorId, [u8; 64])>,
    ...
    /// Deployment-scoped chain-id (#324) used by `on_vote_received` to
    /// reconstruct the canonical `LeaderEndorsement` pre-image for
    /// endorsement verification (#506). Defaults to the all-zero
    /// chain id so direct-construction unit tests behave as they
    /// always have; production wiring routes the genesis-derived
    /// chain id in via [`Self::with_chain_id`].
    chain_id: crate::crypto::signed::ChainId,
    /// Test-only escape hatch (#506): when `true`, skip the
    /// leader-endorsement signature verification in `on_vote_received`
    /// but still run the dedupe / collision-detection logic. Unit
    /// tests that construct hand-rolled votes with
    /// `sig: [0u8; 64]` placeholders cannot mint valid endorsements
    /// without a real signer; the production wiring keeps this `false`
    /// so every wire-driven vote is fully verified before any
    /// `vote_dedupe`/`vote_bucket` mutation.
    skip_endorsement_verification_for_tests: bool,
}
...
impl HotStuffCore {
    ...
    pub fn with_limits(
        ...
        Self {
            ...
            proposal_endorsement_dedupe: HashMap::new(),
            proposal_endorsement_cache: HashMap::new(),
            ...
            chain_id: crate::crypto::signed::ChainId::from_genesis_hash([0u8; 32]),
            skip_endorsement_verification_for_tests: false,
        }
    }

    /// Test-only: skip leader-endorsement signature verification in
    /// `on_vote_received`. Dedupe / collision detection still runs.
    /// Unit tests that build hand-rolled votes with placeholder sigs
    /// (and therefore placeholder endorsements) opt in via this flag;
    /// production callers leave it off (the default).
    #[cfg(test)]
    pub(crate) fn with_skip_endorsement_verification_for_tests(mut self) -> Self {
        self.skip_endorsement_verification_for_tests = true;
        self
    }

    /// Set the deployment-scoped chain id (#324) used by the safety
    /// core's leader-endorsement verification path (#506). The
    /// integration layer calls this once at boot from the genesis-
    /// derived [`crate::crypto::signed::ChainId`]. Defaults to the
    /// all-zero chain id (test sentinel value) so unit tests that
    /// drive the safety core directly behave unchanged.
    pub fn with_chain_id(mut self, chain_id: crate::crypto::signed::ChainId) -> Self {
    ...
    fn build_proposal_at_view(&mut self, view: View) -> Vec<Action> {
        ...
        vec![
            ...
            // The safety core has no signer — `leader_endorsement` is
            // a placeholder envelope that the egress layer overwrites
            // with the real `Signed<LeaderEndorsement>` in
            // `sign_consensus_msg` (#506).
            Action::Broadcast(ConsensusMsg::Proposal(Proposal {
                block: new_block,
                justify: high_qc,
                leader_endorsement: LeaderEndorsement::placeholder(),
            })),
        ]
    }
    ...
    fn on_proposal_received(&mut self, signed: Signed<Proposal>) -> Vec<Action> {
        ...
        let vote_to_broadcast = if safe_to_vote(&signed.payload, &self.state) {
            ...
            // Capture the leader's endorsement from the proposal so the
            // emitted vote can carry it (#506). Voters propagate the
            // endorsement envelope verbatim — they do not re-sign —
            // which is what makes the detection transport-agnostic on
            // the aggregator side.
            Some((
                Vote { view, block_hash },
                signed.payload.leader_endorsement.clone(),
            ))
        } else {
            None
        };
        ...
        if let Some((vote, leader_endorsement)) = vote_to_broadcast {
            actions.push(Action::Broadcast(ConsensusMsg::Vote(
                vote,
                leader_endorsement,
            )));
        }
    ...
    fn on_vote_received(&mut self, variant: VoteVariant) -> Vec<Action> {
        ...
        let leader_endorsement = variant.leader_endorsement().clone();
        let (signed, voter_id, bls_partial): (Signed<Vote>, _, Option<BlsPartialSig>) =
            match variant {
                VoteVariant::Ed25519(verified, _) => {
        ...
        // Leader-endorsement verification (#506). The endorsement is
        // self-attributing — its `signer` field carries the proposer's
        // pubkey — so we don't need to know the cluster's leader
        // selector to look up the right pubkey: the endorsement tells
        // us who claims to have endorsed this `(view, block_hash)`,
        // and we verify the signature against that pubkey. The dedupe
        // map keys on the resulting stable [`ValidatorId`] (resolved by
        // `from_genesis_pubkey`; pre-rotation the wire pubkey IS the
        // stable id, and rotations are handled by the dispatch layer's
        // `ValidatorKeyHistory` resolution before the envelope reaches
        // the safety core).
        let endorsement_key = (vote.view, vote.block_hash);
        let leader_validator_id = if self.skip_endorsement_verification_for_tests {
            // Test fixtures bypass the endorsement check (placeholder
            // envelopes carrying view=0, block_hash=[0;32], signer=[0;32]
            // are common in unit tests). Use `round_robin_leader` as a
            // deterministic fallback so the proposal-equivocation
            // dedupe still keys on a real validator id — this lets
            // tests that intentionally exercise the dedupe path
            // (e.g. votes_from_distinct_signers_for_same_view_*) keep
            // working without standing up real signers.
            let leader_node_id = round_robin_leader(vs_at_vote.for_view(vote.view), vote.view);
            ValidatorId::from_genesis_pubkey(leader_node_id)
        } else {
            // Reject obvious payload mismatches before doing any
            // crypto: the endorsement must be over the same
            // `(view, block_hash)` pair that the vote claims.
            if leader_endorsement.payload.view != vote.view
                || leader_endorsement.payload.block_hash != vote.block_hash
            {
                return vec![Action::VoteRejectedInvalidEndorsement {
                    voter: voter_id,
                    view: vote.view,
                }];
            }
            let claimed_leader_id = ValidatorId::from_genesis_pubkey(leader_endorsement.signer);
            // The claimed leader must be a member of the validator set
            // at `vote.view`; an endorsement signed by a non-member is
            // useless evidence even if its signature verifies.
            if vs_at_vote
                .for_view(vote.view)
                .index_of(&claimed_leader_id)
                .is_none()
            {
                return vec![Action::VoteRejectedInvalidEndorsement {
                    voter: voter_id,
            ...
            let endorsement_already_verified = self
                .proposal_endorsement_cache
                .get(&endorsement_key)
                .is_some_and(|(cached_id, cached_sig)| {
                    *cached_id == claimed_leader_id && *cached_sig == leader_endorsement.sig
                });
            if !endorsement_already_verified
                && leader_endorsement
                    .verify(&leader_endorsement.signer, &self.chain_id)
                    .is_err()
            {
                // Reject before any bucket / dedupe state is touched.
                // The integration layer ticks
                // [`crate::consensus::status::ConsensusStatus::votes_rejected_invalid_endorsement`]
                // off this action.
                return vec![Action::VoteRejectedInvalidEndorsement {
            ...
            if !endorsement_already_verified {
                // Cache so repeat votes on the same
                // `(view, block_hash)` skip the Ed25519 verify call.
                // Bounded the same way `vote_bucket` is bounded —
                // eviction sweeps both maps under the same `gc_below`.
                self.proposal_endorsement_cache
                    .insert(endorsement_key, (claimed_leader_id, leader_endorsement.sig));
            }
            claimed_leader_id
        };
        ...
        // Proposal-equivocation detection (#506): a leader proposes at
        // most one block per view (per the safety core's
        // `proposed_in_view` guard for the local leader). Two distinct
        // `(view, block_hash)` bindings for the same leader extracted
        // from the leader-endorsement field on accepted votes are
        // non-repudiable evidence of a Byzantine leader. Runs **after**
        // the voter-equivocation dedupe so a Byzantine voter who pairs
        // a rejected duplicate vote with a leader-endorsement does not
        // get to inflate the leader-dedupe map; only votes that pass
        // voter-side acceptance can populate this map. Emit before
        // bucket mutation — same emit-before-fold pattern as the voter
        // equivocation path.
        match self
            .proposal_endorsement_dedupe
            .entry((vote.view, leader_validator_id))
        {
            std::collections::hash_map::Entry::Vacant(e) => {
                e.insert(vote.block_hash);
            }
            std::collections::hash_map::Entry::Occupied(e) if *e.get() == vote.block_hash => {
                // Same leader, same view, same block. Idempotent.
            }
            std::collections::hash_map::Entry::Occupied(e) => {
                let block_a = *e.get();
                return vec![Action::ProposalEquivocationEvidence {
                    leader: leader_validator_id,
                    view: vote.view,
                    block_a,
                    block_b: vote.block_hash,
                }];
    ...
    fn evict_vote_buckets_below(&mut self, gc_below: View) {
        ...
        // #506: the proposal-equivocation dedupe map and the
        // verified-endorsement cache share the same view-keyed life
        // cycle as `vote_dedupe` / `vote_bucket`. A leader's
        // proposal-equivocation evidence at a long-past view is no
        // longer actionable (no new bucket can form past `gc_below`,
        // existing buckets at that view are dropped above), so both
        // maps clear under the same `gc_below` cutoff. Without this
        // sweep a Byzantine flood of low-view distinct-hash votes
        // would pin memory in either map.
        self.proposal_endorsement_dedupe
    ...
    fn evict_vote_buckets_to_fit_one(&mut self) {
        ...
        if removed.is_some() {
            // Drop the matching endorsement-cache entry alongside the
            // bucket so the cache stays sized in lockstep with
            // vote_bucket (#506).
            self.proposal_endorsement_cache.remove(&victim_key);
...
fn debug_assert_lock_durable_before_vote(actions: &[Action]) {
    ...
    for (i, action) in actions.iter().enumerate() {
        match action {
            Action::Broadcast(ConsensusMsg::Vote(_, _)) => {
...
mod tests {
    ...
    fn first_proposal_emits_persist_vote_and_adopts_high_qc() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                Action::Broadcast(ConsensusMsg::Vote(
                    Vote {
                        view: View(1),
                        block_hash,
                    },
    ...
    fn byzantine_second_proposer_at_same_view_does_not_extract_a_vote() {
        ...
        assert!(
            first
                .iter()
                .any(|a| matches!(a, Action::Broadcast(ConsensusMsg::Vote(_, _)))),
    ...
    fn lock_persist_precedes_vote_broadcast_for_same_proposal() {
        ...
        let vote_idx = actions
            .iter()
            .position(|a| matches!(a, Action::Broadcast(ConsensusMsg::Vote(_, _))))
    ...
    fn quorum_emits_high_qc_persist_then_broadcast_proposal() {
        ...
        //   2. Broadcast(Proposal { block: new_v4, justify: formed_qc, leader_endorsement: crate::consensus::hotstuff::qc::LeaderEndorsement::placeholder() })
    ...
    /// Distinct signers voting for distinct blocks at *distinct* views
    /// do not trigger voter-equivocation — the dedup key includes the
    /// signer, and crossing a view boundary keeps the
    /// proposal-equivocation dedupe (#506) clear too. Same-view variant
    /// is now covered by
    /// `votes_from_distinct_signers_for_same_view_emits_proposal_evidence`
    /// because two distinct `block_hash` values at the same view
    /// across the leader-endorsement channel is detected as
    /// leader-side proposal equivocation.
    #[test]
    fn votes_from_distinct_signers_at_distinct_views_do_not_emit_evidence() {
        let mut core = make_core(1);
        let block_a: BlockHash = [0xAA; 32];
        let block_b: BlockHash = [0xBB; 32];
        ...
        let two = core.step(Event::VoteReceived(VoteVariant::Ed25519(
            crate::consensus::dispatch::Verified::unchecked(signed_vote(4, block_b, nid(3))),
            crate::consensus::hotstuff::qc::LeaderEndorsement::placeholder(),
        )));
    ...
    /// Two distinct signers voting for two distinct `block_hash` values
    /// at the **same** view bind the leader of that view (resolved via
    /// `round_robin_leader`) to two different blocks via the
    /// leader-endorsement field, which is exactly proposal-equivocation
    /// evidence (#506). Pre-#506 this scenario was the
    /// `votes_from_distinct_signers_for_distinct_blocks_do_not_emit_evidence`
    /// case; post-#506 it is detected on the second arrival as
    /// `Action::ProposalEquivocationEvidence`.
    #[test]
    fn votes_from_distinct_signers_for_same_view_emits_proposal_evidence() {
        let mut core = make_core(1);
        let view: View = View(3);
        let block_a: BlockHash = [0xAA; 32];
        let block_b: BlockHash = [0xBB; 32];
        ...
        assert!(one.is_empty(), "first vote sub-quorum: {one:?}");
        ...
        assert!(
            matches!(
                two.as_slice(),
                [Action::ProposalEquivocationEvidence { view: v, block_a: a, block_b: b, .. }]
                    if *v == view && *a == block_a && *b == block_b
            ),
            "second vote at same view for a different block must emit \
             ProposalEquivocationEvidence (#506): {two:?}",
        );

        // The conflicting vote was dropped before bucket fold, so only
        // the first bucket exists.
        assert_eq!(
            core.vote_bucket
                .get(&(view, block_a))
                .map(|q| q.signer_count()),
        ...
        assert!(!core.vote_bucket.contains_key(&(view, block_b)));
    }
    ...
    mod wal_replay {
        ...
        fn restart_with_locked_qc_refuses_proposal_breaking_lock() {
            ...
            assert!(
                actions
                    .iter()
                    .all(|a| !matches!(a, Action::Broadcast(ConsensusMsg::Vote(_, _)))),
        ...
        fn restart_does_not_re_propose_at_already_proposed_view() {
            ...
            let mut post = HotStuffCore::new(nid(1), post_state, post_builder)
                .with_proposed_in_view(View(4))
                .with_skip_endorsement_verification_for_tests();
