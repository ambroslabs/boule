...
use crate::crypto::signed::{ChainId, Signed, SignedMessage};
...
/// key.
pub(in crate::consensus::dispatch) fn verify_signer_at(
    signer: NodeId,
    view: View,
...
pub(in crate::consensus::dispatch) fn verify_sig<T>(
    signed: &Signed<T>,
    chain_id: &ChainId,
) -> Result<(), IngressError>
where
