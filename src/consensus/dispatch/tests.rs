...
fn egress_commit_returns_none() {
    let signer = fresh_signer();
    let action = crate::consensus::hotstuff::step::Action::Commit(genesis());
    let out = egress_safety(&action, &signer, None, &ChainId::TEST).unwrap();
    assert!(out.is_none());
}
...
fn ingress_with_verify_rejects_tampered_ed25519_qc_inside_proposal() {
    ...
    if let crate::consensus::hotstuff::qc::QcSignatures::Ed25519Collected(sigs) = &mut qc.signatures
    {
        sigs[0][0] ^= 0xFF;
    }
...
fn ingress_with_verify_rejects_bls_qc_on_ed25519_chain() {
    ...
    let real_partial = crate::crypto::sig_scheme::BlsAggregated::sign_partial(&sk, b"x").unwrap();
...
fn make_signed_vote_with_bls_partial(
    ...
    let vote = Vote { view, block_hash };
    let preimage_bytes = crate::crypto::signed::preimage::<Vote>(&vote, &ChainId::TEST).unwrap();
    let partial =
        crate::crypto::sig_scheme::BlsAggregated::sign_partial(bls_sk, &preimage_bytes).unwrap();
    let signed = Signed::sign(vote, signer, &ChainId::TEST).unwrap();
    (signed, partial)
}
